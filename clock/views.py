...
class ClockView(FormView):
    ...
    def get_initial(self):
        current_shift = ClockShift.objects.filter(user=self.request.user, punch_out__isnull=True).first()
        if current_shift:
    ...
    def form_valid(self, form):
        ...
        if action == 'clock_in':
            if not current_shift:
                ClockShift.objects.create(user=self.request.user, punch_in=timezone.now())
...
class ShiftListView(ListView):
    ...
    def get_queryset(self):
        return ClockShift.objects.filter(user=self.request.user)
...
class ShiftCorrectionView(FormView):
    ...
    def get_initial(self):
        shift = get_object_or_404(ClockShift, pk=self.kwargs['shift_id'])
        return {
